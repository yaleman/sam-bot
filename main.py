...
if "logging" in data:
    ...
    LOGFILE_ERROR = data["logging"].get(
        "output_error_file", f"{dir_path}/logs/sambot_error.log"
    )
else:
    # defaults
...
# connecting to MISP
try:
    misp = MispCustomConnector(
        misp_url=data["misp"]["url"],
...
for key in ("SLACK_BOT_OAUTH_TOKEN", "SLACK_SIGNING_SECRET"):
    if key not in data.get("slack"):
        ...
        logger.error(
            "Couldn't find %s in config.json slack section, going to quit.", key
        )
if MISSED_SLACK_KEY:
    sys.exit(1)
else:
...
def handle_message(event_data):
    ...
    # if the incoming message contains 'hi', then respond with a 'hello message'
        ...
        logger.info(
            "Hi message from %s in %s", message.get("user"), message.get("channel")
        )
...
def find_channel_id(
    slack_client_object, channel_name: str = "_autobot"
) -> Optional[str]:
    """returns the channel ID of the channel"""
    for conversation_channel in slack_client_object.conversations_list().get(
        "channels"
    ):
        if conversation_channel.get("name") == channel_name:
            logger.debug(
                "found channel id for %s: %s",
                channel_name,
                conversation_channel.get("id"),
            )
            return conversation_channel.get("id")
    return None
...
if __name__ == "__main__":
    try:
        ...
        if TEST_MODE:
            ...
            BOT_CHANNEL = "bots"
            BOT_CHANNEL_ID = find_channel_id(slack_client, BOT_CHANNEL)
            if BOT_CHANNEL_ID is None:
                logger.error("Failed to find channel #%s, bailing!", BOT_CHANNEL)
                sys.exit(1)

            try:
                slack_client.conversations_join(channel=BOT_CHANNEL_ID)
            except slack.errors.SlackApiError as slack_error:
                logger.error("Failed to join channel: %s", slack_error)
                sys.exit(1)
